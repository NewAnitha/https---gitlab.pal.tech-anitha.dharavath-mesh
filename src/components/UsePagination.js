...
export const usePagination = ({ totalCount, rowsPerPage, siblingCount, presentPage }) => {
  const paginationList = useMemo(() => {
    ...
    if (paginationListCount < pagesRequired) {
      ...
      const rightSiblingIndex = Math.min(presentPage + siblingCount, pagesRequired);
      ...
      if (shouldShowLeftDots && !shouldShowRightDots) {
        ...
        const rightRange = range(pagesRequired - rightItemCount + 1, pagesRequired);
